...
where 
\begin{equation}
  \xi(\bm{t}, w^{-1}) = \sum_{p=1}^\infty t_p w^{-p}\; ,
\end{equation}
...
For later convencience, let us also define an invertible matrix
\(\mathcal{K}\) by its components
\begin{equation}
    \mathcal{K}_{jk} = 
\oint_{\gamma_k} \frac{d w}{2\pi i} e^{\xi(\bm{t}, w^{-1})} \frac{w^{-j}\mathcal{Y}_k(w)}{w - v_k}\;,  
\end{equation}
so that the tau function is simply \(\bm{\tau}(\bm{t}, \bm{v}) = \det\mathcal{K}\).
...
\begin{equation}
  ...
  \end{split}
\end{equation}
and we see that these functions have simple poles at \( \lambda = z_j\). 
