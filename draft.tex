...
functions, we must introduce the Miwa coordinates, which will be
discussed in more detail in Section~6. The key point for now is that
the homogeneous limit corresponds to the following specialization of
the Miwa coordinates:
\begin{equation}
  t_p = \frac{n}{p} w^p\;, \quad p = 1, 2, 3, \dots
\end{equation}
...
variable \(x\).  Finally, we can use standard techniques to construct
a solution to the KP equation in the form
\begin{equation}
  \varphi_{KP} = 2\, \partial_{t_1}^2 \ln \bm{\tau}_h\;,
\end{equation}
and investigate the behavior of this solution near the Bethe roots,
its spectral and singular structure. However, the key point is that
these aspects require further analysis before we can address such
specific details. In particular, we aim to construct the
Baker--Akhiezer function in a more general setting.
