...
\title[Slavnov products, KP and BA functions]{More on Algebraic Bethe
  Ansatz Scalar Products as KP Hierarchy Tau Functions}
...
\begin{abstract}
 ...
 analyzed from the viewpoint of partially off-shell scalar products of
 Bethe states, the so called Slavnov products. It is well known that,
 modulo model dependent aspects, the functional structure of Slavnov
 products generally takes the form of determinants. Building on recent
 results on the structure of rational and trigonometric models, we
 show that, provided certain conditions are satisfied, the Slavnov
 products of quantum integrable systems can be interpreted as tau
 functions of the KP hierarchy, thus extending known results in a more
 general setting.  In light of this, we begin by analyzing the
 classical features of these tau functions, particularly the types of
 classical solutions defined by the Slavnov products. We show that
 Slavnov products can be expanded in terms of other tau functions.  We
 also show that their homogeneous limit yields a Wronskian-type
 solution of the KP hierarchy, where the Wronskian is constructed from
 functions related to the eigenvalues of the transfer matrices.
 Finally, we compute the Baker-Akhiezer functions associated with
 these tau functions and show that, apart from a universal
...
function in quantum integrable systems, the so called \emph{Slavnov
product}~\cite{Slavnov1989}. These objects can be seen as building
blocks of more sophisticated correlation functions and are
...
of perspectives~\cite{Wu:1975mw, Its:1992bj, korepin2000the, Foda2009,
  Alexandrov:2011aa, Zabrodin:2012vj, Alexandrov:2013hca}.
In~\cite{Foda:2009zz}, the authors started the analysis of the
connections between the Slavnov products of the Heisenberg XXZ spin
chain and the Kadomtsev–Petviashvili (KP) classical integrable
hierarchy. Their analysis has been extended in several different
directions and applications, for example~\cite{Wheeler:2010vmq,
  Foda:2010, Takasaki:2010qm, Foda:2012wn, Foda:2012wf}.
In~\cite{Araujo:2021ghu, Araujo:2024klz}, the author of the current
paper also investigated some aspects of this research problem. In
particular, the relation between integrable hierarchies and quantum
integrable systems such as the Q-Boson integrable system and the
...
investigated this property for models with rational and trigonometric
\(R\)-matrices, and basically answered ``\emph{why scalar products in
the Algebraic Bethe Ansatz have determinant representations}''. Using
their results, we now want to understand ``\emph{why and how (some)
scalar products in the Algebraic Bethe Ansatz are KP and Toda tau
functions}''. We prove that, as long as the integrable system
satisfies the conditions established in~\cite{Belliard:2019bfz} --
along with some additional restrictions -- the Slavnov product is
guaranteed to satisfy the KP integrable hierarchy.

Having established this result, we proceed to analyze the classical
aspects of the tau functions defined via Slavnov products. Our initial
observation is that these tau functions differ from those studied
in~\cite{Segal:1985aga, Kharchev:1991cy, Alexandrov:2014cwa}, whose
origin lies in matrix model analysis. We also explore the possibility
of a solitonic origin; to this end, we compute the homogeneous limit
and find that it defines a Wronskian-type solution of the tau
function~\cite{Kodama2018}. Finally, to further investigate its
nature, we begin the computation of the Baker-Akhiezer function using
...
Section 4 discusses the homogeneous limit of the Slavnov product,
which is relevant for investigating the solitonic aspects of these tau
functions. In this limit, all complex parameters condense to a single
value, and we show that the tau functions define a Wronskian solution
of the KP integrable hierarchy, where the Wronskian is constructed
from functions related to the eigenvalues of the transfer
matrix. Section~5 presents explicit examples, and we observe that even
in these cases, the expressions for the tau functions become
overwhelmingly complex very quickly.
...
The following result was established by Belliard and Slavnov~\cite{Belliard:2019bfz}:
...
\end{equation}
All in all, we use these determinants to normalize the \(n+1\)
\emph{Slavnov products} as follows
\begin{equation}
...
\end{equation}
This concludes our discussion of~\cite{Belliard:2019bfz}. The
expression given in~(\ref{eq:slavnov-tau}) is the one that interests
us most.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Slavnov products as KP tau functions}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Given the expression~(\ref{eq:slavnov-tau}), our goal now is to show
that these functions satisfy the Hirota bilinear equations, which
essentially means that they are tau functions of the KP
hierarchy. Similar calculations have been carried out in the past,
particularly in the original works of Foda and
collaborators~\cite{Foda:2009zz, Wheeler:2010vmq, Foda:2010,
  Takasaki:2010qm, Foda:2012wn, Foda:2012wf}.  Their strategy involved
expressing the objects in terms of fermionic bilinears and
demonstrating that they can be written as sums over Schur polynomials,
with coefficients satisfying the Plücker relations.

In this section, we adopt a simpler approach. We
rewrite~(\ref{eq:slavnov-tau}) as an alternant determinant and invoke
the results of~\cite{Segal:1985aga, Kharchev:1991cy} which guarantee
that such expressions, in general, satisfy the Hirota bilinear
equations. In essence, by rewriting the Belliard--Slavnov formula for
the Slavnov product \(\bm{\tau}^{(\ell)}(\bm{u}_\ell; \bm{v})\) as an
alternant determinant, we make manifest that it defines a tau function
of the KP hierarchy.  We conclude this section with a proof that the
KP tau functions discussed above can be expressed in terms of other
tau functions of the KP hierarchy.

In order to proceed, let us simplify the notation and organize the
results described above. First of all, let us use the
components~(\ref{eq:omega-matrix-def}) to define \(n\) functions \(\Omega_j(z)\) of the
form
\begin{equation}
\begin{split}
\label{eq:om-def}
...
Before proceeding, let us make two interesting observations. The first
is a (bold) conjecture regarding a hypothetical multicomponent KP
hierarchy underlying all the calculations discussed above. The second
is a proof that the KP tau functions above can be expressed in terms
of other tau functions of the KP hierarchy.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Conjecture: Multicomponent tau functions}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Wronskian-type KP tau functions}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

In the previous section, we mentioned that given a set of generic
functions \(\{\phi_j\}_{j=1}^n\), it has been shown that functions of
the form
\begin{equation}
\label{eq:general-tau}
\bm{\tau}(\bm{w}) = \frac{\det_{i,j} \phi_i(w_j)}{\Delta(\bm{w})}\;,
\end{equation}
are tau functions of the KP hierarchy. This proposition has been
extensively discussed in the literature, see for
example~\cite{Segal:1985aga, Kharchev:1991cy, Araujo:2021ghu} and
references therein. In~\cite{Segal:1985aga, Kharchev:1991cy,
  Alexandrov:2014cwa}, the authors explored families of functions \(\{
\phi_i \}_{i=1}^n\) that parametrize points in a Grassmannian space.

These functions also play an important role in the definition of the
Baker--Akhiezer functions. However, the asymptotic behavior of the
functions \(\Omega_j\), which are central to our discussion, suggests
that they do not belong to the class considered in the references
above. Therefore, the main goal in the remainder of this work is to
identify general aspects of the classical tau functions associated
with the Slavnov products.

In this section, we investigate the homogeneous limit of these
objects, and we find that this limit yields a Wronskian-type solution
of the KP hierarchy; see~\cite{Kodama2018}. It is interesting to
contrast this result with the solitonic solutions of the hierarchy,
which can also be expressed in terms of Wronskians. To connect this
solution to the standard KP hierarchy framework, we will need to
introduce Miwa coordinates.
...
In order to better capture the meaning of these Wronskian-type tau
functions, we must introduce 
\begin{equation}
  \label{eq:miwa-coord}
  \bm{t} = (t_1, t_2, t_3, \dots) \; , \qquad 
  t_p = \frac{1}{p}\sum_{j=1}^n z_j^p\; ,
\end{equation}
the so-called Miwa coordinates, which will be discussed in more detail
in Section~6. The key point for now is that the homogeneous limit
corresponds to the following specialization of the Miwa coordinates:
\begin{equation}
  t_p = \frac{n}{p} w^p\;. 
\end{equation}
Under this identification, we find that the Wronskian-type solution of
the KP hierarchy can be written as
\begin{equation}
  \bm{\tau}_h(t_1, \bm{v}) =
  \frac{1}{\prod_{k=1}^n(n^{-1} t_1 - v_k)}\,
  \mathcal{W}[\mathcal{Y}_1, \mathcal{Y}_2, \dots, \mathcal{Y}_n](n^{-1}t_1)\;,
\end{equation}
where the coordinate \( t_1 \) can be interpreted as the spatial
variable \(x\).

Alternatively, one can follow the techniques developed in the original
references~\cite{Foda:2009zz, Wheeler:2010vmq, Foda:2010,
  Takasaki:2010qm, Foda:2012wn, Foda:2012wf} to express the tau
function~(\ref{eq:slav-tau-function}) as a linear combination of Schur
polynomials \( s_{\lambda}(\bm{z}) \), or written in terms of Miwa
coordinates \( s_{\lambda}(\bm{t}) \):
\begin{equation}
  \bm{\tau}(\bm{z}, \bm{v}) = \sum_{\lambda \subseteq [n, L]} c_\lambda\, s_{\lambda}(\bm{z})
  \quad  \textrm{or} \quad 
 \bm{\tau}(\bm{t}, \bm{v}) = \sum_{\lambda \subseteq [n, L]} c_\lambda\, s_{\lambda}(\bm{t})\;,
\end{equation}
where \(\lambda = (\lambda_1, \lambda_2, \dots, \lambda_n)\), with \(L
\geq \lambda_j \geq \lambda_{j+1}\), denotes a Young Tableaux
contained in a box of size \([n, L]\). Moreover, the coefficients \(
c_\lambda \) can be explicitly written in terms of the functions \(
\Omega_j(z)\).  The homogeneous limit discussed above corresponds to
the specialization
\begin{equation}
  s_\lambda(w, w, w, \dots, w) = w^{|\lambda|} s_\lambda(1, 1, 1, \dots, 1)\;,
  \quad |\lambda| = \sum_{j=1}^n \lambda_j\;,
\end{equation}
and in this limit, the Schur polynomial \( s_\lambda(1, 1, 1, \dots,
1) \) counts the number of \emph{semi-standard Young tableaux} of
shape \(\lambda\), see~\cite{Stanley2023}.  Therefore, we obtain a
purely combinatorial expression for the tau functions, with weights
given by the Plücker coordinates \( c_\lambda \), that is
\begin{equation}
  \bm{\tau}_h(w, \bm{v}) = \sum_{\lambda \subseteq [n,L]} w^{|\lambda|} c_\lambda\, (\#\mathrm{SSYT})_\lambda\;.
\end{equation}

Before proceeding, let us calculate some examples to better visualize
...
In Section 4, we began investigating certain features of the Slavnov
products from the perspective of the KP hierarchy. In particular, we
examined the behavior of these tau functions when all parameters
condense to a single value. Using known techniques, we found that the
resulting tau functions are of Wronskian type, and we also observed
that, when expressed in terms of Schur polynomials, this limit admits
a combinatorial interpretation.

In this section we want to continue with this investigation, but now
without taking any particular limit for the parameters. Here we
address some properties of the Baker-Akhiezer functions associated
with the tau functions derived above. While many aspects of these
functions deserve thorough examination, here we focus on their most
essential properties.  This section presents two main findings: first,
we derive explicit expressions for the Baker--Akhiezer function;
second, based on its mathematical structure, we argue that it defines
a soliton-like solution, in the sense described
in~\cite{Zabrodin2018}.
...
the Miwa coordinates~(\ref{eq:miwa-coord}). We first define the
function
\begin{equation}
...
It was first established in~\cite{ZinnJustin:2002cr,
  ZinnJustin:2002pk} that tau functions of the alternant
form~(\ref{eq:general-tau}) admit an integral representation; see
also~\cite{Zinnjustin2009, Araujo:2021ghu} for further discussion. Let
us now use these results to express~(\ref{eq:slav-tau-function}) in
integral form. For pedagogical purposes -- and to clarify several
differences in our conventions -- we provide a complete proof of this
result below.

\begin{proposition}
\label{prop:integral-rep}
The tau functions in the alternant form~(\ref{eq:slav-tau-function})
...
In terms of the \(z\)-coordinates, the
integral~(\ref{eq:tau-function-miwa}) becomes
\begin{equation}
...
\end{equation}
The determinant above is itself a tau function, but we can also write
it as
\begin{equation}
    \det \widehat{\bm{\mathcal{K}}} = 
    \frac{\det(\bm{\Omega})}{\Delta(\bm{z})}
...
\begin{equation}
  \begin{split}
    ...
    & = \exp\left[ \sum_{k=1}^\infty \frac{(-1)^{k-1}}{\lambda^k} q_k\right] \; ,
  \end{split}
\end{equation}
where we define the coordinates as 
\begin{equation}
q_k = \frac{1}{k} \sum_{l=1}^n \mu_l^k\; .
\end{equation}

Then, we write the Baker-Akhiezer as 
\begin{equation}
\label{eq:ba-nindependent}
  \psi(\bm{t},\bm{v}; \lambda) = e^{\xi(\bm{t}, \lambda)}
  \left(1 + \sum_{k\geq 1} \frac{\sigma_k(\bm{q}(\bm{t}))}{\lambda^k} 
  \right)\; .
\end{equation}
...
\bm{q} \), that is, \( \xi_k(\bm{t}) \equiv \sigma_k(\bm{q}) \), where
the parameter \( n \) is now implicitly contained in their definition.

Let us now briefly discuss the meaning of these expressions. We first
observe that the Baker--Akhiezer function~(\ref{eq:ba-standard}) is in
standard form (see~\cite{Zabrodin2018}), where all coefficients \(
\xi_k(\bm{t}) \) are elementary symmetric polynomials in the
eigenvalues of the matrix \( \bm{\mathcal{M}} \). The corresponding KP
field can then be obtained from
\begin{equation}
  u = -2\, \partial_{t_1} \xi_1(\bm{t})\;.
\end{equation}

In this form, it becomes clear that these solutions define a
generalization of the soliton-like solutions of the KP hierarchy
discussed in~\cite{Zabrodin2018}. In particular, we identify \( n \)
functions
\begin{equation}
  \varrho_k(\lambda) = \frac{\mathcal{Y}_k(\lambda)}{\lambda - v_k}\;, \qquad k = 1, \dots, n\;,
\end{equation}
such that
\begin{equation}
  \oint_{\gamma_k} \frac{d\lambda}{2\pi i}\, \psi(\bm{t}, \bm{v}; \lambda^{-1})\, \varrho_k(\lambda) = 0\;, 
\end{equation}
so that the tau-function~(\ref{eq:tau-function-miwa}) is simply
\begin{equation}
  \bm{\tau}(\bm{t}, \bm{v}) =
    \det_{j,k}\left(
    \oint_{\gamma_k} \frac{d \lambda}{2\pi i} e^{\xi(\bm{t}, \lambda^{-1})} \lambda^{-j}\varrho_k(\lambda) \right)\; ,
\end{equation}
This construction extends the soliton-like solutions described in~\cite{Zabrodin2018}.

Let us finish this section with two important observations. 

\begin{remark}

First, let us observe that the functions \( \mathcal{Y}_k(\lambda) \)
in~(\ref{eq:om-def}) are defined by the absence of the Bethe root \(
v_k \). As a result, there is no limiting procedure in \( \lambda \)
that connects different functions \( \mathcal{Y}_k(\lambda) \). Our
goal is to find an appropriate embedding of this system into the Sato
Grassmannian. In the case of the XXX model, Belliard and Slavnov have
shown in~\cite{Belliard:2019bfz} that the functions \(
\mathcal{Y}(\lambda) \) are polynomials of degree \( 2n + 1 \).

Let us consider the sum
\begin{equation}
  \sum_{k=1}^n \mathcal{Y}_k(\lambda) = a_{2n+1} \lambda^{2n+1} + \mathcal{O}(\lambda^{2n})\;,
\end{equation}
where \( a_{2n+1} \) is the coefficient of the highest-degree term and
depends on the Bethe roots \( \bm{v} \). We then define the function
\begin{equation}
  \phi_1(\lambda) = \frac{1}{a_{2n+1}} \sum_{k=1}^n \mathcal{Y}_k(\lambda)
  = \lambda^{2n+1} \left( 1 + \mathcal{O}(\lambda^{-1}) \right)\;.
\end{equation}
Next, we define \( n-1 \) additional functions \( \phi_j(\lambda) \)
as
\begin{equation}
  \phi_j(\lambda) = \lambda^{j-1} \phi_1(\lambda)\;, \quad j = 2, 3, \dots, n\;.
\end{equation}
By comparison with the results of~\cite{Kharchev:1991cy}, we expect
that, at least in this case, the set of functions \( \{
\phi_j(\lambda) \} \) provides coordinates on the Sato
Grassmannian. In future work, we hope to present a more rigorous
justification of this heuristic argument, as well as a clearer proof
that this embedding is appropriate.
\end{remark}

\begin{remark}
In this work, we have restricted ourselves to the analysis of finite
\( n \). However, the limit \( n \to \infty \) is fundamental for a
deeper understanding of the embedding of these solutions within the
classical KP hierarchy framework. One advantage of the integral
representation~(\ref{eq:tau-function-miwa}) and the Baker--Akhiezer
function~(\ref{eq:ba-nindependent}) is that both retain an implicit
dependence on \( n \), making them well-suited for studying this
limit. The limit \( n \to \infty \) is expected to correspond to the
thermodynamic limit of the spin chain, and therefore provides a
natural setting for our problem.

A key point in our analysis is that the tau functions defined by the
Slavnov products correspond to restricted tau functions. This is
clearly seen when we expand the tau function in terms of Schur
polynomials, as done in the original references~\cite{Foda:2009zz,
  Wheeler:2010vmq, Foda:2010, Takasaki:2010qm, Foda:2012wn,
  Foda:2012wf}, and briefly reviewed in Section~4. In that context,
the corresponding Young tableaux are confined to a box of size \([n,
  L]\). To obtain a fully general tau function of the KP hierarchy, we
must consider the limit \( n \to \infty \), \( L \to \infty \).

In this limit, the tau function \( \bm{\tau}(\bm{t}) \) becomes a
function of infinitely many time variables \( \bm{t} = (t_1, t_2,
\dots) \) and defines a point in the Sato Grassmannian, thus
corresponding to a full KP hierarchy. In terms of the Schur polynomial
expansion, this means summing over Young tableaux with arbitrarily
long hooks. Interestingly, the tau function itself appears to have a
trivial expression in this limit (see, for
example,~\cite{Babbitt1990}). 

On the other hand, the analysis of the Baker--Akhiezer function in
this limit is more subtle. In particular, we need to better understand
the Fredholm determinant expression~(\ref{eq:ba-fredholm}). A central
issue is to prove that the matrix appearing in this expression defines
a trace-class operator and to characterize the structure of its
kernel; it may also be necessary to further impose that \( n / L \to 0
\). These considerations suggest that, in the thermodynamic limit, the
Baker--Akhiezer function can indeed be represented as a Fredholm
determinant. We are currently investigating this question and hope to
report new results in the near future.

One of the key advantages of this formulation is that it provides a
more natural framework for taking the thermodynamic limit, as the
dependence on \( n \) becomes implicit in all relevant expansions.
\end{remark}
...
In this work, we have discussed classical properties of the Slavnov
products arising in quantum integrable models and their deep
connection with the tau functions of the KP hierarchy. Our main
findings can be summarized as follows. Using the general form of the
Slavnov products, we argue that these objects can be firmly
established as tau functions within the framework of the classical KP
...
The behavior of these functions near the Bethe roots of quantum
integrable systems, as well as the homogeneous limit of the Slavnov
product, has also been discussed. The latter aspect allows us to
identify the tau function in this limit as a Wronskian-type solution
of the KP hierarchy. Finally, to shed further light on the nature of
these tau functions, we conclude our work with a brief analysis of the
Baker--Akhiezer function. Our most important result in this context is
the observation that the Baker--Akhiezer function, modulo a universal
multiplicative factor, also admits a determinantal representation.
...
Another important aspect of this work is to gain a deeper
...
objects, and a complete description of their properties is still
lacking. We are currently investigating the limit \( n \to \infty \),
which corresponds to a proper Sato and Segal--Wilson tau
function. This limit should coincide with the thermodynamic limit of
the Slavnov product, and we hope that known results in the literature
will help address some of the open questions.

In particular, it is important to understand the significance of the
Slavnov product from the perspective of the Baker--Akhiezer
function. For instance, how does one properly describe the double
limit \( L \to \infty \) and \( n \to \infty \), with \( n \) growing
sufficiently slower than \( L \)? We believe that the matrix \(
\mathcal{M} \) is a trace-class operator, which would allow the
Baker--Akhiezer function to be expressed as a Fredholm determinant. We
hope to report new results along this line of investigation soon.
...
are known for the partially on-shell scalar product of Bethe
states~\cite{Slavnov:2020xxj}, it is not yet clear whether these
objects are also related to tau functions of integrable hierarchies.
This characterization is another problem we are currently
investigating, and we hope to have some results to report in the
future.
