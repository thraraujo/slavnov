...
\paragraph{\emph{Proof}} Let us first use that
\begin{equation}
...
This section deals with the properties of the Baker-Akhiezer functions
associated to the above tau functions. There are some interesting
aspects of these tau functions that deserve further analysis, and here
we just touch some of these aspects.

Let us first start with some rewriting of the tau functions above. 
...
\begin{proposition}
The tau function~(\ref{eq:tau-function}) admits an integral
representation, see ~\cite{Araujo:2021ghu} and references therein,
of the form
\begin{subequations}
\begin{equation}
...
original references; namely, the integration contour \(\gamma_k\) and
sign conventions. For this reason, we want to show
explicitly this result.
...
\(\bm{\tau}(\bm{z}, \bm{v}) = \det\bm{\mathcal{K}}\). \qed
